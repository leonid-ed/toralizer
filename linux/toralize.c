...
int kProxyPort;
int kSocksVer;
int (*_func_connect)(int, const struct sockaddr *, socklen_t) = NULL;

void __attribute__((constructor)) initializer(void)
{
  printf("%s: the library has been loaded\n", LOG_PREFIX);

  _func_connect = dlsym(RTLD_NEXT, "connect");
  if (!_func_connect) {
    printf("%s: failed to load the function 'connect()'\n", LOG_PREFIX);
    return;
  }
...
int connect(int original_socket, const struct sockaddr *address,
            socklen_t addrlen)
{
  ...
  if (_func_connect(proxy_socket, (struct sockaddr *)&sock, sizeof(sock))) {
